...
from bootcamp_data.io import read_users_csv, read_orders_csv, write_parquet

from bootcamp_data.quality import require_columns, assert_non_empty, assert_unique_key
from bootcamp_data.transforms import (
    enforce_schema_orders,
    enforce_schema_users,
    add_missing_flags,
    normalize_text,
    apply_mapping,
    parse_datetime,
...
def transform(orders_raw: pd.DataFrame, users: pd.DataFrame) -> pd.DataFrame:
    ...
    orders = (
        orders_raw.pipe(enforce_schema_orders).assign(
    ...
    if len(joined) != len(orders):
        raise ValueError("Row count changed after join")
